...
def dinic_algorithm(graph: Graph, source: int, sink: int) -> int:
    def build_level_graph(level_graph, source):
        ...
        while queue:
            ...
            for next_node in graph.get_nodes_from_node(current_node):
                if next_node not in level_graph:
    ...
    def blocking_flow(current_node, target, flow, level_graph):
        ...
        current_flow = 0
        for next_node in graph.get_nodes_from_node(current_node):
            if level_graph.get(next_node, -1) == level_graph[current_node] + 1:
                edge_capacity = graph.get_edge_stat(current_node, next_node)[0]
                delta_flow = blocking_flow(
                    next_node,
                    target,
                    min(flow, edge_capacity),
                    level_graph
                )
                if delta_flow > 0:
